...
function save_auth_token() {
    var api_token_file = path.join(configdir, "auth_token.json");
    try {
        fs.writeFileSync(api_token_file, JSON.stringify({"token": auth_token}));
    }
    catch(err) {
        console.error("Could not save auth token to [%s]: %s", api_token_file, err);
    }
}
