...
function establish_tunnels(options) {
    ...
    get_host_info(opts, function(err) {
        ...
        opts.noit_creds = noit.utils.hashToCreds(opts.creds);
            ...
            body.forEach(function(bundle) {
                if ( bundle._checks && bundle.config && bundle.config.port ) {
                    bundle._checks.forEach(function(check) {
                        console.log("adding tunnel for check %s", bundle._cid);
...
function get_broker_hostname(brokerid, callback) {
    if ( brokerid in broker_hosts ) {
        if ( broker_hosts[brokerid] == null ) {
            console.log("checking back in a sec");
    ...
    api.get(brokerid, {}, function(code, err, body) {
        ...
        if ( body && body._details && body._details[0].cn ) {
            ...
            console.log("brokerid %s has hostname %s", brokerid, broker_hosts[brokerid]);
            return callback(null, broker_hosts[brokerid]);
        }
    });
}
...
function reverse_tunnel_for_check(check_id, port) {
    api.get(check_id, {}, function(code, err, body) {
        ...
        get_broker_hostname(body._broker, function(err, hostname) {
            ...
            console.log("adding reverse tunnel: hostname %s, broker host %s, port %s, check %s", opts.target, hostname, port, check_id);
...
/*
    console.dir(check_id);
    if ( !broker_hosts[bundle._] ) {
        // calls back to this function when done fetching hostname
        get_broker_hostname(bundle._broker, function() { reverse_tunnel_for_check(bundle); });
        return;
    }


    opts.ports.forEach(function(portnum) {
        console.log("adding reverse tunnel for host %s, port %s, check %s", opts.broker_host, portnum, check._cid);
        proxy.reverse(check_id, "127.0.0.1", portnum);
    });
*/
