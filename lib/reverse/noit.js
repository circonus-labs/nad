...
class Connection extends EventEmitter {
    ...
    _start(cb) {
        ...
        this.socket.on('timeout', () => {
            ...
            log.debug('stop and restart reverse due to socket timeout');
            if (self.socket) {
                log.trace('call socket.end to trigger close');
                self.socket.destroy(new Error('socket timeout'));
            } else {
    ...
    _stop() {
        ...
        if (this.socket !== null && !this.socket.destroyed) {
            log.debug('closing reverse socket');
            this.socket.end();
        }
    ...
    commWatchdog() {
        ...
        if (!stateOK) {
            ...
            if (this.socket !== null && !this.socket.destroyed) {
                this.socket.destroy(new Error('watchdog timeout'));
            }
