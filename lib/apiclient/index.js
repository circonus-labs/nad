    ...
    do_request(options, cb) {
            ...
            res.on('end', () => {
                ...
                if (res.statusCode === 429 || res.statusCode === 500) {
                    if (options.circapi.retry < options.circapi.retry_backoff.length) {
                        log.warn({ status_code: res.statusCode }, 'retrying API call');
                        setTimeout(() => {
                    ...
                    log.error({ status_code: res.statusCode, retries: options.circapi.retry }, 'giving up after max retries with API call failures');
                    cb(
        ...
        req.on('error', (err) => {
            if (err.code === 'ECONNRESET' && options.circapi.retry < options.circapi.retry_backoff.length) {
                log.warn({ err: err.message }, 'retrying API call');
            ...
            log.error({ err: err.message }, 'giving up after API call failure(s)');
            cb(
                err,
                null,
                null,
