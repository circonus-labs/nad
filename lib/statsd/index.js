...
function getFlushTimeout(interval) {
    let timer_interval = interval - (Date.now() - settings.statsd.start_time) % settings.statsd.flush_interval; // eslint-disable-line max-len, no-mixed-operators

    if (timer_interval < 0) {
        log.debug(`calculated negative flush timer_interval (${timer_interval}), resetting to ${settings.statsd.flush_interval}...`);
...
function startServer(cfg, callback) {
    const servermod = require(path.join(__dirname, 'lib', 'servers', cfg.server)); // eslint-disable-line global-require
