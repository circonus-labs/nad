...
function getMetricDest(name) {
    // no host or group key settings, send all metrics to host
    if (settings.statsd.group_key === null && settings.statsd.host_key === null) {
        return 'host';
    }

    // explict settings, both group and host keys set
    if (settings.statsd.group_key !== null && settings.statsd.host_key !== null) {
        if (name.startsWith(settings.statsd.group_key)) {
            return 'group';
        }
        if (name.startsWith(settings.statsd.host_key)) {
            return 'host';
        }

        return null; // ignore, explicit matches only
    }

    if (settings.statsd.group_key !== null) {
        if (name.startsWith(settings.statsd.group_key)) {
            return 'group';
        }

        return 'host'; // default to host
    }

    if (settings.statsd.host_key !== null) {
        if (name.startsWith(settings.statsd.host_key)) {
            return 'host';
        }
...
function removeDestFromName(name, dest) {
    if (dest === 'group' && (settings.statsd.group_key !== null && name.startsWith(settings.statsd.group_key))) {
        return name.substr(settings.statsd.group_key.length);
    }

    if (dest === 'host' && (settings.statsd.host_key !== null && name.startsWith(settings.statsd.host_key))) {
