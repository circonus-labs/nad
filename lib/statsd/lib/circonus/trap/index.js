 ...
 * @arg {String} cid check bundle id
...
function fetchBroker(cid) {
    ...
    return new Promise((resolve, reject) => {
        ...
        client.get(cid, null, (err, body, code) => {
            if (err !== null) {
                reject(new Error(`${errPrefix} ${err.message} ${body} ${code}`));
 ...
 * @arg {Object} bundle check bundle definition
 * @returns {Object} promise
 */
 ...
 * @arg {String} urlProtocol ^(http|https):$
 * @returns {String|null} proxy url if found, otherwise null.
 */
 ...
 * @arg {Object} metrics from check bundle
 * @returns {Object} inventory of metrics indicating active status
 */
...
module.exports = class Trap {
     ...
     * @arg {String} check_type ^(host|group)$
     */
    ...
    initialize() {
        ...
        return new Promise((resolve) => {
            ...
            if (!settings.statsd.manage_check_metrics && self.submission_url !== null) {
                self.enabled = true;
            ...
            self._enableMetrics(metrics).
                ...
                then((numMetrics) => {
                    if (numMetrics >= 0) {
                        ...
                        self.log.debug({
                            latency     : `${diff[0]}s ${(diff[1] / 1e6).toFixed(2)}ms`,
     ...
     * @arg {String} metric name
     * @returns {Boolean} indicating whether to activate metric
     */
    _activateMetric(metric) {
        ...
        if (!this.metrics[metric]) {
            return settings.statsd.force_metric_activation;
        }
     ...
     * @arg {Object} metrics to enable
     * @returns {Object} promise
     */
    _enableMetrics(metrics) {
        if (!settings.statsd.manage_check_metrics) {
     ...
     * @returns {Object|null} options for http[s] request
     */
    _getSubmitOptions() {
