...
Executable plugins:
    Place excutable file in plugin_dir (default: /opt/circonus/nad/etc/node-agent.d)

    Plugin should emit lines in one of the following three formats:
    ...
    The JSON document should be of the form:
...
Examples can be found at https://github.com/circonus-labs/nad/blob/master/examples/plugins/
...
class Plugins {
    ...
    constructor(push_receiver) {
        ...
        if (settings.debug_dir !== null && dutil === null) {
            try {
                dutil = require('nad/debug_util'); // eslint-disable-line global-require
            } catch (err) {
    ...
    run(req, res, which) {
        ...
        function send_complete() {
            ...
            log.trace({ metrics }, 'sending metrics');
    ...
    _exec_plugin(plugin, plugin_instance, args, req, cb) { // eslint-disable-line max-params
        ...
        if (plugin.is_native) {
            plugin.native_obj.run(plugin,
                                  (_plugin, _metrics, _plugin_instance) => {
                                      log.trace({ metrics: _metrics, plugin: _plugin_instance }, 'received metrics');
        ...
        function handle_output(_plugin, _cb, _plugin_instance) {
            ...
            log.trace({ metrics: results, plugin: _plugin_instance }, 'received metrics');
