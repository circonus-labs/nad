...
function setAPIOptions(options) {
    ...
    if (!(/^[a-f0-9-]{4,}$/).test(instance.api.key)) {
...
function parseCommandLineOptions() {
    ...
    options.
        ...
        option('--no-nad-metrics', `Don't send nad metrics (memory, uptime, cpu)`).
        //
        ...
        option('--debugdir <path>', `Create debug files for each plugin and write to this directory []`).
    ...
    instance.send_nad_stats = options.nadMetrics;
...
class Settings {
    ...
    constructor() {
        ...
        if (instance.reverse.enabled) {
            if (!instance.api.key || instance.api.key === '') {
                const msg = 'Reverse enabled, no API key supplied';

                console.error(instance.pfx_error, msg);
                log.fatal(msg);
                process.exit(1);
            }
        }
