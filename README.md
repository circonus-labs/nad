...
nad is an portable, extensible, leightweight metric collection
agent. It is the recommended way to collect system metrics
for the [Circonus](Circonus.com) monitoring platform.

nad comes with a [rich set of plugins](https://github.com/circonus-labs/nad/tree/master/plugins) that collect
- system metrics on Linux, Solaris, FreeBSD and OpenBSD, and
...
Further applications can be added easily using a simple but powerful
...
Node.js v4.0.0 or later is must be installed and available as `node`
on the PATH.
...
There are deamon install targets for some operating systems, which enable
all the default checks and install init scripts and default configuration
helper files. More details below.
